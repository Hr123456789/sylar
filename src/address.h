...
public:
    ...
    //addr sockaddr指针
    //addrlen sockaddr的长度
    //反悔和sockaddr相匹配的Address, 失败返回nullptr
    ...
    //type socket类型SOCK_STREAM, SOCK_DGRAM等
    ...
    //通过host地址返回对应条件的任意Address 智能指针形式
    //host 域名，服务器名等，举例：www.sylar.top[:80](方括号为可选内容)
    //family 协议族(AF_INT, AF_INT6, AF_UNIX)
    //type socket类型SOCK_STREAM, SOCK_DGRAM等
    //protocol 协议，IPPROTO_TCP、IPPROTO_UDP等
...
public:
    ...
    static IPAddress::ptr Create(const char* addrees, uint16_t port = 0);
    ...
    //prefix_len 子网掩码位数
    //return 调用成功返回IPAddress,失败返回nullptr
    virtual IPAddress::ptr subnetMask(prefix_len) = 0;
